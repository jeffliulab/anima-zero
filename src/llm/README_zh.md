...
(`LLM` 协议),不关心背后是 Claude、OpenAI 还是本地 Ollama。所以**加一个新大脑,编排器、世界、前端都不用动**。
...
| `mock.py` | `MockLLM` —— 剧本式假脑,不用 key、不用网也能把整条链路跑通 |
...
- `image_png` —— 当前画面(可能是 `None`,表示没连世界、纯聊天)。
...
ANIMA 是**具身机器人的大脑**,连上世界后每一轮都会拿到画面(`image_png`)。所以新大脑**必须是视觉模型**
(`vision = True`,并真的把 `image_png` 喂进去)。纯文本模型(只会读字、看不了图)**不要接**——
它在"连了世界、要看着画面操作"的场景里没法用。
