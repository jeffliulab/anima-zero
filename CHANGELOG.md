...
5. **会话核心任务**（大脑侧唯一改动，通用机制）：耐力跑实测出「任务滑出上下文窗口 → 大脑中途停下汇报」——「当前在执行什么任务」是**状态**不是聊天记录。会话新增核心任务寄存器：LLM 经两个内建元工具 `set_core_task`/`clear_core_task` **亲自**登记/改写/清除（不做任何关键词/钉死/自动启发式），常驻注入系统提示（状态通道，不占历史窗口）。回合制不变：ANIMA 每完成一手就停下等用户；靠寄存器，用户只说「我走完了」它就知道该干嘛。
