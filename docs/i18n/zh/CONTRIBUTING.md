...
三类贡献最欢迎,各给一条「先读哪个文件」的路径:

- **写一个新世界**——最有价值的一种。从 `src/examples/minimal_world.py` 入手
  (一条约 300 行的走廊,逐行对照规范注释),再读 [`world/README_zh.md`](../../../world/README_zh.md)
  搞清「两条线」的规矩,写完用 `anima conformance <url>` 自查。世界是你自己的程序——
  不一定要并进本仓,发布在任何地方、告诉我们一声,同样是贡献。
- **审校翻译**(见 [ROADMAP R8](ROADMAP.md))——日语、法语、西语文档都是维护者在模型协助下写的,
  **从没被母语者读过**。如果你是,`docs/i18n/` 就在那;哪怕只改对几句话也是真修复。
- **文档与小修小补**——哪句话让你多花了五分钟,它也会让下一个人多花五分钟。标着
  **`good first issue`** 的 issue 都是「不需要懂全仓也能做完」的粒度。
