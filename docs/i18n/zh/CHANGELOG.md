...
Main：**没有 API key 也能把整套跑起来**——随包的世界、本地 CPU 大脑，以及从 demo 走出去的每一条命令。同时仓库做好了待客准备：一份 FAQ、一扇给贡献者的前门、安全告警归零，以及那些曾经静默发错东西的发包链路缺陷被修复并守进 CI。

1. **`anima demo` 回归，且当年删它的三样代价一样都不带回。** 一个约 300 行的走廊世界（一个点、`look`、`step`、一张真相机画面）随包分发，照 AWI 规范手写——没有逐字节副本、没有 submodule、没有每张世界清单里多一个 desk，正是这三样杀死了 v1.1 的 desk 世界。demo 在空闲端口起它，并明说着挑大脑：有 API key 就用你的；没有就用**本地 CPU 大脑**（Qwen3-4B-Instruct，经 Ollama，tag 是 `qwen3:4b-instruct`，约 2.5GB——最小且工具调用真正可靠的尺寸）；再不行才落诚实的 mock。这条走廊同时就是「写你自己的世界」的模板（`src/examples/minimal_world.py`，`python -m anima.examples.minimal_world` 可单独跑）。配套地，主循环现在尊重 `llm.vision`：不长眼睛的脑不再被塞画面。
2. **无 key 那条路是通到底的，不只通到 demo。** 有三处把它断开了，都是发版前把独立审计员放进来才逮到的。`anima world add` 审批完世界却不把它注册到任何地方，于是 demo 紧接着印的那条命令直接报「No world named …」——现在它会把世界**追加**进 `ANIMA_WORLDS`（追加、绝不替换；`--no-save` 可以不写）。`anima chat`、`anima run` 和 `POST /api/sessions` 只要配置的默认脑没有 key 就当场抛鉴权错，现在三处都会落到第一个真能用的脑并明说换了谁。还有网页的传感区：随包的走廊世界明明跑得好好的，它却报「未连接，确认世界进程有没有起来」——一个世界完全可以只实现 AWI 那四条通道而不提供视频流，这现在是它自己的一种平静状态，不再是告警。已在**完全没有 key** 的环境里端到端实测：demo、注册与审批世界、终端对话、网页应用连同 AWI 仪表盘，全部由 CPU 上的 4B 大脑驱动跑通。
3. **装出来的包，数据落在你找得到的地方。** `pip install` 过去把日志、会话记忆、以及最要命的那份「文档让你新建的」`.env` 全写进 `site-packages`——等于 pip 用户**根本没有地方配 API key**。现在这些统一落 `~/.anima`（`ANIMA_HOME`），和信任决定早就采用的做法一致；源码检出的行为一个字没变。`anima doctor` 会直接告诉你东西都落在哪。
4. **新用户路径的其余清障。** gazebo-chess 移出默认世界清单（代码早已住进伴随仓；设 `GAZEBO_CHESS_URL` 就回来，不管 `ANIMA_WORLDS` 设没设）；`.build-time` 时间戳终于进了 wheel，`anima serve` 对 pip 用户报告的是网页的真实构建时间，且 CI 每次推送都证明 wheel 带的是新界面和时间戳；新增 `docs/faq.md`（中英双语）；`/awi` 和 `/session-logs` 直接打开不再 404——还各自补上了语言切换器（ROADMAP R9 销账）。
5. **安全告警归零**（ROADMAP R4 销账）：Next 15 → 16，`postcss`/`sharp` 用 `overrides` 钉过公告线，并带上 Next 16 要求的 tsconfig 改写。`npm audit` 干净；大版本跳之后界面已逐页肉眼复核。
6. **给贡献者的前门**：CONTRIBUTING 新增「从哪里入手」一节（照走廊模板写世界 / 审校翻译 / good first issue），外加 `CITATION.cff` 和 PyPI 徽章。

## [1.1.1] — 2026-07-30

Main: desk 世界和 `anima demo` 都删掉了。那个随 wheel 分发的世界，存在的理由是让 `pip install` 之后有个去处；代价是两份逐字节副本、一个 git 子模块、以及每张世界清单里都有两个 desk——不值。

1. **两个 desk 世界连同 `anima demo` 一起删**：随 wheel 走的那个内置世界，和它旁边的 `sim-desk` 子模块。⚠️ **`pip install anima-zero` 现在给你的是大脑，一个世界都没有。** 世界是独立的程序：clone 本仓能拿到 `world/` 下那几个，或者照 AWI 规范自己写一个。mock 大脑没动，照样能选。`anima doctor` 在清单为空时会说清去哪弄一个世界，而不是留一张沉默的空表。
2. **README 把你交给 coding agent**：原先安装那一节炫的是「一条命令就能看」，现在改成说清下一步是去弄一个世界，以及最快的上手方式是把仓交给 Claude Code 或 Codex、让它读 `AGENTS.md`。
3. **一致性检查不再拿「我们碰巧带了哪个世界」当靶子**：端到端那条改成在**测试内部照规范手写一个最小世界**——八格走廊、一个工具、一张真 PNG——并且把那张图真的解码回来，而不是信它自称的 mime 类型。它不绑 `world/` 下任何一个世界，所以对「只装了大脑、自己写世界」的人同样成立。⛔ 靶子起不来时它红、不是 skip；被它替换掉的那条是 skip，那会把「世界已经不存在了」变成一次绿色的运行。
4. **本仓不再有任何子模块**，`awi_mcp.py` 从六份逐字节副本降到四份，其中只剩 soma-zero 那一份在本仓之外。
...
> ⚠️ **已被 v1.1.1 取代**：内置 desk 世界与 `anima demo` 已删除。`anima` 命令、随 wheel 分发的网页、mock 大脑均不受影响。
...
5. **语言分层**：模型读的一切（系统提示词、工具描述、各状态块、各世界说明书）统一为**英文单一版本**，收口到 `src/prompts.py`，末尾一句"用用户说话的语言回复"；人读的文档出中英两份；CLI 与网页默认英文。⚠️ **这是一次行为变更，而能给出结论的基准还没重跑**——五房间导航的前后对比是欠账，明写在 ROADMAP R2。在那之前"英文更好"是一个**假设**，不是结论；真要更差，回退点是一个文件。⚠️ **第一遍漏了很多**——编排器拼进系统提示词的世界块、每帧随图发出的画面说明、安全闸的三条理由、截断提示、后端送给网页的每一条错误、以及 mock 大脑自己的回复，在这句话已经写进 CHANGELOG 的时候**都还是中文**。它们是**靠装上 wheel 跑起来**发现的，不是靠读源码。剩下什么、为什么剩，见 ROADMAP R6。
