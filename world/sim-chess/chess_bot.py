# ⚠️ 角色：sim-chess 世界自带的「内置电脑对手」引擎——世界专用副本。
# services/boardgame_engine/chess_engine.py 是另一份独立副本（ANIMA 大脑的棋理顾问）。两份【禁止去重合并】：
# 对手与顾问是两个不同角色，零共享代码是有意为之的边界——关掉引擎服务(:8108)，世界内置电脑照走；
# 日后可各自独立调强弱（对手调弱做陪练、顾问保持最强）。
# 源自原外部 3-anima-chess-engine/chess/engine.py（v0.6 内聚搬入，算法未改）。
"""
