...
class HouseSim:
    ...
    def drive_turn(self, degrees: float, sign: int) -> dict:
        ...
        ⚠️ **有的机器人转弯时要带一点前进速度才迈得动腿**——带多少由资产库清单里的
        `turn_vx` 说了算（那是各台机器人策略的客观特性，实测出来的，⛔ 别在这里写死）。
        当前两台都是 0：四足狗原地打转照样迈腿；人形自 2026-07-27 换上「上游对齐包」策略后
        也能真正原地转了（屋内闭环转 90° 实测 2.30 秒、40°/s、前后漂移 0.06 m）。
        历史：上一版人形策略站着几乎不转（3.8°/s），只能靠 `turn_vx=0.3` 绕过去，
        代价是转 90° 往前挪 0.6~0.8 米。**位移无论多少都如实报给大脑，不假装没动。**
        """
