...
- **64 格全可达**:几何对齐真实摆位(4.5cm 格、底座轴心到板边 10cm),抓取姿态改**径向倾斜**
  (朝远离基座方向倒,抓取半径 0.44m→0.53m)——v0.5 遗留的「h 列整列不可达」就此修复。
- **整盘跑法**:世界摆标准开局 FEN、后端 `ANIMA_MAX_STEPS` 调大,一句「把这盘棋下完」——
  观察-思考-行动的唯一主循环连续走完全局;每盘自动落档,`eval/` 记分卡按世界分指标评分。

## 状态

**v0.7(Pre-alpha),持续迭代中。** v0.1 封版了顶层架构(世界独立 + 会话 + 主循环 + 外围 hook + 原生 tool-calling);
