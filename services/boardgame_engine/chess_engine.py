# ⚠️ 角色：ANIMA 大脑的「棋理顾问」引擎——boardgame-engine service 专用副本。
# world/sim-chess/chess_bot.py 是另一份独立副本（世界自带的电脑对手）。两份【禁止去重合并】：
# 顾问与对手是两个不同角色，零共享代码是有意为之的边界（日后可各自独立调强弱，如对手调弱做陪练）。
